...
class SupervisedRegistrationManager(RegistrationManager):
    ...
    def delete_expired_users(self):
        """
        Remove expired instances of ``SupervisedRegistrationProfile`` and their
        associated ``User``s.
        ...
        1. It alleviates the ocasional need to reset a
           ``SupervisedRegistrationProfile`` and/or re-send an activation email
           when a user does not receive or does not act upon the
           initial activation email; since the account will be
           deleted, the user will be able to simply re-register and
           receive a new activation key.
        ...
        If you have a troublesome ``User`` and wish to disable their
        account while keeping it in the database, simply delete the
        associated ``SupervisedRegistrationProfile``; an inactive ``User`` which
        does not have an associated ``RegistrationProfile`` will not
        be deleted.
