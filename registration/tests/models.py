...
class SupervisedRegistrationModelTests(TestCase):
    ...
    def test_profile_creation(self):
        ...
        self.assertEqual(SupervisedRegistrationProfile.objects.count(), 1)
        self.assertEqual(profile.user.id, new_user.id)
        self.failUnless(re.match('^[a-f0-9]{40}$', profile.activation_key))
        self.assertEqual(six.text_type(profile),
                         "Registration information for alice")
    ...
    def test_activation_email(self):
        """
        ``SupervisedRegistrationProfile.send_activation_email`` sends an
        email.
    ...
    def test_user_creation(self):
        ...
        """
        new_user = SupervisedRegistrationProfile.objects.create_inactive_user(
            site=Site.objects.get_current(), **self.user_info)
        self.assertEqual(new_user.username, 'alice')
        self.assertEqual(new_user.email, 'alice@example.com')
        self.failUnless(new_user.check_password('swordfish'))
    ...
    def test_user_creation_email(self):
        ...
        """
        SupervisedRegistrationProfile.objects.create_inactive_user(
            site=Site.objects.get_current(), **self.user_info)
        self.assertEqual(len(mail.outbox), 1)
    ...
    def test_unexpired_account(self):
        """
        ``SupervisedRegistrationProfile.activation_key_expired()`` is ``False``
        within the activation window.

        """
        new_user = SupervisedRegistrationProfile.objects.create_inactive_user(
            site=Site.objects.get_current(), **self.user_info)
        profile = SupervisedRegistrationProfile.objects.get(user=new_user)
        self.failIf(profile.activation_key_expired())
    ...
    def test_valid_admin_approval(self):
        ...
        user = SupervisedRegistrationProfile.objects.admin_approve_user(
    ...
    def test_activation_deactivated(self):
        ...
        # Try to activate again and ensure False is returned.
        failed = SupervisedRegistrationProfile.objects.activate_user(
            profile.activation_key, Site.objects.get_current())
        self.assertFalse(failed)
    ...
    def test_expired_user_deletion(self):
        """
        ``SupervisedRegistrationProfile.objects.delete_expired_users()`` only
        deletes inactive users whose activation window has expired.

        """
        SupervisedRegistrationProfile.objects.create_inactive_user(
            site=Site.objects.get_current(), **self.user_info)
        expired_user = (SupervisedRegistrationProfile.objects
                        .create_inactive_user(
                            site=Site.objects.get_current(),
                            username='bob',
                            password='secret',
        ...
        SupervisedRegistrationProfile.objects.delete_expired_users()
        self.assertEqual(SupervisedRegistrationProfile.objects.count(), 1)
        self.assertRaises(UserModel().DoesNotExist,
                          UserModel().objects.get, username='bob')
    ...
    def test_resend_activation_email(self):
        ...
        profile = SupervisedRegistrationProfile.objects.get(user=user)
        orig_activation_key = profile.activation_key

        self.assertTrue(SupervisedRegistrationProfile.objects.resend_activation_mail(
            email=self.user_info['email'],
            site=Site.objects.get_current(),
        ))

        profile = SupervisedRegistrationProfile.objects.get(pk=profile.pk)
        new_activation_key = profile.activation_key
    ...
    def test_resend_activation_email_nonexistent_user(self):
        ...
        Test resending activation email to a nonexisting user
        """
        self.assertFalse(SupervisedRegistrationProfile.objects.resend_activation_mail(
            email=self.user_info['email'],
            site=Site.objects.get_current(),
        ))
        self.assertEqual(len(mail.outbox), 0)
