...
class RegistrationFormTests(TestCase):
    ...
    def test_registration_form_tos_and_unique_email(self):
        """
        Test that ``RegistrationFormTOSAndUniqueEmail`` validates
        both uniqueness of email and terms of service agreement
        """
        UserModel().objects.create_user('alice', 'alice@example.com', 'secret')

        form = forms.RegistrationFormTOSAndUniqueEmail(data={'username': 'foo',
                                                          'email': 'alice@example.com',
                                                          'password1': 'foo',
                                                          'password2': 'foo'})
        self.failIf(form.is_valid())
    ...
    def test_registration_form_tos_and_no_free_email(self):
        """
        Test that ``RegistrationFormUniqueAndNoFreeEmail`` validates
        terms of service agreement and disallows registration
        with free email addresses.
        """
        base_data = {'username': 'foo',
                     'password1': 'foo',
                     'password2': 'foo'}
        for domain in forms.RegistrationFormTOSAndNoFreeEmail.bad_domains:
            invalid_data = base_data.copy()
            invalid_data['email'] = "foo@%s" % domain
            form = forms.RegistrationFormTOSAndNoFreeEmail(data=invalid_data)
            self.failIf(form.is_valid())
            self.assertEqual(form.errors['email'],
                             ["Registration using free email addresses is prohibited. Please supply a different email address."])

        base_data['email'] = 'foo@example.com'
        form = forms.RegistrationFormTOSAndNoFreeEmail(data=base_data)
        self.failIf(form.is_valid())
        self.assertEqual(form.errors['tos'],
                         ["You must agree to the terms to register"])
        base_data['tos'] = 'on'
